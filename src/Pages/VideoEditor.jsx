...
class VideoEditor extends Component {
    ...
    if (popupWindow) {
      popupWindow.document.write(`
        ...
        <style>
          ...
          .banner { background:#ecfdf5; padding:10px; border-radius:8px; margin-bottom:10px; display:none; }
          .delete-btn { background: transparent; color: #ef4444; border: none; cursor: pointer; font-size: 18px; padding: 4px; border-radius: 4px; }
                ...
                for(let i=0; i<count; i++) {
                  ...
                  div.innerHTML = \`<div>\${fmt(s.timestamp/1000)} - \${fmt(en.timestamp/1000)} [\${s.zone||''}] \${q?'(Queued)':''} \${sv?'(Saved)':''}</div>
                    <div style="display:flex; gap:8px;">
                      ...
                      \${(!q && !sv)?\`<button onclick="window.opener.handleCreatePendingSegmentFromPopup(\${i})">+ Queue</button>\`:''}
                    </div>\`;
  ...
  setupPopupHandlers = () => {
    ...
    window.deleteTagPairFromPopup = async (sid, eid) => {
      ...
      if(s&&en) {
        // Find matching segment on timeline
          ...
          <EnhancedTimeline segments={segments} duration={duration} currentTime={this.getMatchTime(currentTime)} matchStartTime={matchStartTime}
            ...
            onDeleteSegment={async id=>{
              ...
              if(s){
                ...
                if(st)await videoTagService.delete(st.id); 
                if(et)await videoTagService.delete(et.id);
              } 
              await videoSegmentService.delete(id); 
              await this.loadTags(); 
              await this.loadSegments();
            }}
          />
