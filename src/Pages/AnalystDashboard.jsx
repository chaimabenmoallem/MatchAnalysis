...
export default function AnalystDashboard() {
  ...
  const handlePopOutAnnotation = () => {
    ...
    <body>
      ...
      <script>
        ...
        const formData = {
          ...
          actionStart: ${JSON.stringify(actionStart)},
          actionEnd: ${JSON.stringify(actionEnd)}
        };
            ...
            html = \`
              ...
              <div style="margin-top: 16px;">
                ...
                <div id="goalMapContainer" style="position: relative; width: 100%; padding-bottom: 33.33%; background: #e2e8f0; border-radius: 8px; border: 4px solid #cbd5e1; overflow: hidden; cursor: crosshair;">
                  <!-- Net pattern SVG -->
                  <svg style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; z-index: 1;" viewBox="0 0 100 33" preserveAspectRatio="none">
                    <!-- Vertical lines -->
                    <line x1="5" y1="0" x2="5" y2="33" stroke="#cbd5e1" stroke-width="0.5"/>
                    <line x1="10" y1="0" x2="10" y2="33" stroke="#cbd5e1" stroke-width="0.5"/>
                    <line x1="15" y1="0" x2="15" y2="33" stroke="#cbd5e1" stroke-width="0.5"/>
                    <line x1="20" y1="0" x2="20" y2="33" stroke="#cbd5e1" stroke-width="0.5"/>
                    <line x1="25" y1="0" x2="25" y2="33" stroke="#cbd5e1" stroke-width="0.5"/>
                    <line x1="30" y1="0" x2="30" y2="33" stroke="#cbd5e1" stroke-width="0.5"/>
                    <line x1="35" y1="0" x2="35" y2="33" stroke="#cbd5e1" stroke-width="0.5"/>
                    <line x1="40" y1="0" x2="40" y2="33" stroke="#cbd5e1" stroke-width="0.5"/>
                    <line x1="45" y1="0" x2="45" y2="33" stroke="#cbd5e1" stroke-width="0.5"/>
                    <line x1="50" y1="0" x2="50" y2="33" stroke="#cbd5e1" stroke-width="0.5"/>
                    <line x1="55" y1="0" x2="55" y2="33" stroke="#cbd5e1" stroke-width="0.5"/>
                    <line x1="60" y1="0" x2="60" y2="33" stroke="#cbd5e1" stroke-width="0.5"/>
                    <line x1="65" y1="0" x2="65" y2="33" stroke="#cbd5e1" stroke-width="0.5"/>
                    <line x1="70" y1="0" x2="70" y2="33" stroke="#cbd5e1" stroke-width="0.5"/>
                    <line x1="75" y1="0" x2="75" y2="33" stroke="#cbd5e1" stroke-width="0.5"/>
                    <line x1="80" y1="0" x2="80" y2="33" stroke="#cbd5e1" stroke-width="0.5"/>
                    <line x1="85" y1="0" x2="85" y2="33" stroke="#cbd5e1" stroke-width="0.5"/>
                    <line x1="90" y1="0" x2="90" y2="33" stroke="#cbd5e1" stroke-width="0.5"/>
                    <line x1="95" y1="0" x2="95" y2="33" stroke="#cbd5e1" stroke-width="0.5"/>
                    <!-- Horizontal lines -->
        ...
        // Apply theme on load if category is pre-selected
        if (selectedCategory) {
          const container = document.querySelector('.container');
          container.className = 'container theme-' + selectedCategory;
        }
    ...
    if (popupWindow) {
      ...
      // Listen for messages from popup (only once)
      window.addEventListener('message', (e) => {
        if (e.data.type === 'annotationData') {
          ...
          if (data.actionStart !== null && data.actionEnd !== null && data.selectedCategory && 
            ...
            const absoluteStartMs = Math.round((currentSegment.start_time / 1000 + data.actionStart) * 1000);
            const absoluteEndMs = Math.round((currentSegment.start_time / 1000 + data.actionEnd) * 1000);
            ...
            console.log('Auto-saving annotation from expanded view:', payloadData);
        ...
        <CardContent className="p-4 bg-slate-900 space-y-4">
          ...
          {annotations.length > 0 && (
            <div className="border-t border-slate-700 mt-6 pt-6 space-y-4">
              <div className="text-sm font-semibold text-slate-300">Timeline</div>

              {/* Timeline Tracks */}
              <div className="space-y-2">
                {actionCategories.map((cat) => {
                  const catAnnotations = annotations.filter(a => a.action_category === cat.id);
                  ...
                  const count = catAnnotations.length;
                  ...
                  return (
                    <div key={cat.id} className="flex gap-3 items-center">
                      ...
                      {/* Timeline Track */}
                      <div className="flex-1 relative bg-slate-800 rounded h-6 border border-slate-600 overflow-hidden">
                        {catAnnotations.length === 0 ? (
                          ...
                          catAnnotations.map((ann, idx) => {
                            const segmentStartTime = currentSegment ? currentSegment.start_time / 1000 : 0;
                            const relativeStartTime = ann.start_time / 1000 - segmentStartTime;
                            const relativeEndTime = ann.end_time / 1000 - segmentStartTime;
                            ...
                            return (
                              <div
                                key={ann.id || idx}
                                className="absolute h-full rounded hover:opacity-90 transition-opacity cursor-pointer flex items-center justify-center text-center"
                                style={{
                                  left: `${startPercent}%`,
                                  width: `${widthPercent}%`,
                                ...
                                title={`${formatTime(relativeStartTime)} - ${formatTime(relativeEndTime)}`}
                              >
